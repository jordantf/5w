...
inline std::string spritesheet_path(const std::string& name) { return data_path() + "/spritesheets/" + name; };
...
// Cursor struct so everything can identify the cursor
struct Cursor {};
...
struct TilePosition {
	...
	// all movable characters are Solid
	// powers are not solid
	bool isSolid = true;
    bool lit = false;
};
