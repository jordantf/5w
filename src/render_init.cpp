...
vec2 RenderSystem::camera_offset = {0,0};
	...
	window(window)
{
    ...
    // create frame buffer for shadow
    shadowframe_buffer = 128;
    glGenFramebuffers(1, &shadowframe_buffer);
    // initialize texture
    glGenTextures(1, &rendered_shadow);

    preprocessing_buffer = 512;
    glGenFramebuffers(1, &preprocessing_buffer);
    glGenTextures(1, &rendered_screen);
    shadow_sprite = ShadedMesh();
    shadow_sprite.effect.load_from_file(shader_path("tri") + ".vs.glsl", shader_path("tri") + ".fs.glsl");
...
RenderSystem::~RenderSystem()
{
    ...
    glDeleteFramebuffers(1, &shadowframe_buffer);
    glDeleteFramebuffers(1, &preprocessing_buffer);
...
// Create a new sprite and register it with ECS
void RenderSystem::createSprite(ShadedMesh& sprite, std::string texture_path, std::string shader_name, bool has_normal)
{
	if (texture_path.length() > 0) {
        if (has_normal) {
            std::cout << "texture path: " << texture_path + " normal.png\n";
            sprite.normal.load_from_file(texture_path + " normal.png");
        }
        sprite.texture.load_from_file(texture_path + ".png");
    }
	// The position corresponds to the center of the texture.
	TexturedVertex vertices[4];
	vertices[0].position = { -1.f/2, +1.f/2, 0.f };
...
void RenderSystem::initScreenTexture()
{
	...
	createSprite(screen_sprite, "", "sprite_lit");
    createSprite(process_sprite, "", "post_processing");

	// Initialize the screen texture and its state
	screen_sprite.texture.create_from_screen(&window, depth_render_buffer_id.data());
    //process_sprite.texture.create_from_screen(&window, depth_render_buffer_id.data());
	auto& state = ECS::registry<ScreenState>.emplace(screen_state_entity);
	state.bloom_factor = 0;
