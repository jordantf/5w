...
struct ScreenState
{
	float bloom_factor = 0.6;
    float bloom_radius = 0.5;
    float brightness = 0.4; // from 0.0 (pure black) to 1.0 (pure white), 0.5 = default
    float saturation = 0.5; // from 0.0 (greyscale) to 1.0 (boosted)
    float chrom_abb = 0.2;
};
...
// A struct to refer to entities that have taken damage.
struct Hurt
{
	// The amount of time before the main counter begins being decremented
	// Default value: 0
	float delayCounterMs = 100;

	// The initial value of the main timer, to allow for comparisons.
	float initCounterMs = 1000;

	// The main timer. Any future effects should check against this time.
	float counterMs = 1000;

	// The health of the character after it receives the damage.
	// This field is to ensure that while decrementing HP during the timer, it doesn't go under this value.
	int finalHealth;

	// The amount of damage done to the entity. Can be used to display damage numbers.
	int damage;

	// If the damage will KO the entity, set this to be true __when the component is initialized for the entity__.
	bool death;
};
...
struct TempText {
    ...
    float init_counter;
    vec2 movement;
    TempText(float c, vec2 move = {0,0});
};