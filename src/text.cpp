...
public:
    FreeTypeContext()
        ...
        outline_textShader.load_from_file("data/shaders/outlined_text.vs.glsl", "data/shaders/outlined_text.fs.glsl");
    }
    ...
    Effect& outlineShader() noexcept {
        return outline_textShader;
    }
...
void drawText(const Text& text, glm::vec2 gameUnitSize) {
    ...
    Effect& shader = text.outline? ctx.outlineShader() : ctx.textShader();
	...
	if (text.outline) {
        glUniform3f(
                glGetUniformLocation(shader.program, "outline_color"),
                text.outline_color.x,
                text.outline_color.y,
                text.outline_color.z
        );
        glUniform1f(
                glGetUniformLocation(shader.program, "outline_thickness"),
	...
	for (const auto& c : u32str) {
        ...
        if (c == '\n') {
            ...
            cursor.y -= ch.Size.y * text.scale * 1.5;
        } else { // hardcoded for attack
            // compute the on-screen texture coordinates from the cursor's
            // baseline origin
            const auto xpos = cursor.x + ch.Bearing.x * text.scale + offset.x;
            const auto ypos = cursor.y + (ch.Bearing.y - ch.Size.y) * text.scale - offset.y;
            auto w = (ch.Size.x + offset.x) * text.scale;
            auto h = (ch.Size.y + offset.y) * text.scale;

            float advance = ch.Advance;

            // use custom icon for resource characters
            Texture t;
            if(c == '@' || c == '#' || c == '|' || c == '}') {
                if (c == '@') t.load_from_file(textures_path("ui/spell_resources/attack.png"));
                if (c == '#') t.load_from_file(textures_path("ui/spell_resources/move.png"));
                if (c == '|') t.load_from_file(textures_path("ui/spell_resources/spell.png"));
                if (c == '}') t.load_from_file(textures_path("ui/spell_resources/key.png"));
                w = h; // our icons are square
                advance = 2450; // set custom advance
                glBindTexture(GL_TEXTURE_2D, t.texture_id); // bind texture
            } else {
            ...
            cursor.x += advance / 64.0f * text.scale;
        }
	}
	glBindVertexArray(0);
