...
// BFS Pathfinding AI with PUSHING

class Enemy6AI : public EnemyAI {
    public:
    ...
    Enemy6AI(ECS::Entity enemy_p, WorldSystem *world_p) : EnemyAI(enemy_p, world_p) {
        enemy = enemy_p;
        world = world_p;
    }
    ...
    void beginTurn() override  {
        ...
        if (closest_player_pos.x != -1 && !no_paths) { // closest_player_pos x value set to -1 if no player found in range.
            if (inAttackRange(closest_player_pos, tile_position.grid_pos) && turn.attacks > 0) {
                ...
                dude.castSpell(enemy, *PUSH, closest_player_pos);
                turn.spells--;
                world->step_turn();
                no_paths = false;
...
protected:
    ...
    Spell* PUSH = new Spell(Spell::SpellEffect::PUSH, 1, 1, 0, 4, true, true, false);
