...
// BFS Pathfinding AI with trigger

class Enemy7AI : public EnemyAI {
    public:
    ...
    Enemy7AI(ECS::Entity enemy_p, WorldSystem *world_p) : EnemyAI(enemy_p, world_p) {
        enemy = enemy_p;
        world = world_p;
    }
    ...
    void beginTurn() override  {
        ...
        Character& ch = ECS::registry<Character>.get(enemy);
        ...
        if (closest_player_pos.x != -1 && !no_paths) { // closest_player_pos x value set to -1 if no player found in range.
            if (inAttackRange(closest_player_pos, tile_position.grid_pos) && turn.attacks > 0) {
                ...
                world->step_turn();
                no_paths = false;
            } else if (triggered && turn.movement > 0) {
...
//                turn.movement--;
            ...
            } else if (!triggered && (fabs(closest_player_pos.y-tile_position.grid_pos.y
            + (closest_player_pos.x-tile_position.grid_pos.x)) <= 2)) {
                triggered = true;
            } else if (ch.currentHealth < ch.maxHealth) {
                triggered = true;
            } else {
